...
def getHeader(logurl="/login/login", username="", loginData={}):
    print(
        "=> login into system! (account = " + username + ") (logurl = " + logurl + ")"
    )
...
if (
    __name__ == "__main__"
):  # 直接运行文件时执行（如果文件是被导入到其他文件中，`__name__`将被设置为文件名）
    access_token = login(loginPostData)
    # 获取当前执行文件所在目录的绝对路径
    configFilePath = os.path.dirname(os.path.abspath(__file__)) + "/config.py"
    c.put(
        configFilePath, "access_token", access_token
    )  # 更新全局令牌，方便在后面的请求中带上token
