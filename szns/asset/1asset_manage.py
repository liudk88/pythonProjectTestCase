...
l=[]
l.append(FunClass("listAsset","【查询资产】"))
# l.append(FunClass("addAsset","【新增资产】"))
l.append(FunClass("updateAsset","【修改资产】"))
l.append(FunClass("delAsset","【下架】"))
l.append(FunClass("infoAsset","【资产详情】"))
l.append(FunClass("addSystem","【新增信息系统】"))


g_assetId='1232365110391996419'

def listAsset():
    c.pget("/asset/view/AssetView-BS/view?SYS_NAME=zhengquanjun&flag=1")
...
# zhangjingjing2账号测试信息系统权限功能
def addSystem():
    ## 测试新增主系统、子系统的数据权限
    postData={"SYS_NAME": "zhengquanjun-mainSystem", "IS_IT_A_SUBSYSTEM": "1", "ASSET_TYPE": "BS", "ASSET_TYPEC": "BS", "ASSET_TYPEH": "BS", "FILL_ORG": "1456-232P41LH8OF", "SYS_STATE": "0"}
    # c.ppost("/asset/form/AssetForm-BS",postData)
    ## select * from asset_topology at2 order by CREATE_DATE desc;   有记录

    postData={"SYS_NAME": "zhengquanjun-subSystem", "IS_IT_A_SUBSYSTEM": "0", "ASSET_TYPE": "BS", "ASSET_TYPEC": "BS", "ASSET_TYPEH": "BS", "FILL_ORG": "1456-232P41LH8OF", "SYS_STATE": "0"}
    postData['MAIN_SYSTEM']='1239631698279268352'
    c.ppost("/asset/form/AssetForm-BS",postData)
    ## asset_topology无记录

    ## 模糊查询应该只有一条数据，因为zhengquanjun-subSystem没有设置主系统，子系统权限应该和主系统一样
    res=c.pget("/asset/view/AssetView-BS/view?SYS_NAME=zhengquanjun&flag=1")
    print("查询结果数量："+str(len(res.json()['data']['datas'])))
